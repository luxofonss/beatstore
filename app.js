...
var audio = $("#audio");
var forward = $(".playing-btn-forward");
...
var app = {
  ...
  handleEvents: function () {
    var isPlaying = false;
    var playPauseBtn = $$(".beat-item-img");
    var playingBtnFooter = $(".playing-btn-play");
    var pauseBtnFooter = $(".playing-btn-pause");
    var range = $("#myRange");
    var currSongId = 0;
    var _this = this;

    Array.from(playPauseBtn).forEach(function (song, index) {
      console.log(
        playPauseBtn
          .item(currSongId)
          .querySelector(":nth-child(1) > :nth-child(2)")
      );
      var btnSelector = song.querySelector(":nth-child(1) > :nth-child(2)");
      // console.log(btnSelector);
      audio.src = _this.beats[currSongId].src;

      var playSong = function () {
        console.log("play");
        isPlaying = true;
        playingBtnFooter.classList.add("hidden");
        pauseBtnFooter.classList.remove("hidden");
        pauseBtnFooter.classList.add("active-icon");
      ...
      var pauseSong = function () {
        isPlaying = false;
        playingBtnFooter.classList.add("active-icon");
        playingBtnFooter.classList.remove("hidden");
        pauseBtnFooter.classList.add("hidden");
      ...
      var playSong2Song = function () {
        ...
        btnSelector2 = playPauseBtn
          .item(currSongId)
          .querySelector(":nth-child(1) > :nth-child(2)");

        btnSelector2.innerHTML = `
            <div class="play-btn hidden">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd"
      ...
      playingBtnFooter.onclick = function () {
        playSong();
      };

      pauseBtnFooter.onclick = function () {
        pauseSong();
      };

      playPauseBtn[index].onclick = function () {
        audio.src = _this.beats[index].src;
        var playPauseBtnParent = song.parentElement;
        if (isPlaying === false) {
          playSong();
        } else {
          if (currSongId !== index) {
